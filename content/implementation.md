...
The test cases are ran using 10-fold cross validation. First, we extracted the phase and timing of signal groups and randomly divided these in 10 groups. Then we replayed every signal group update $$i$$ from a SPAT message, who belongs in one group, and predicted its duration $$d_p$$ using the frequency distributions made from the other 9 groups and compared it with its real duration $$d$$. Only signal group updates where its duration is unknown (minimum and maximum duration differ) are considered.
